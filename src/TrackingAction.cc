...
if (track->GetTouchableHandle()->GetVolume() == fDetector->GetAbsorber() ) {
...
/*	G4cout << " pozicija vertex " << G4BestUnit(pos,"Length") << " ir energija: " << G4BestUnit(energy,"Energy") << G4endl;
   ...
   FILE * output=fopen("antrines.txt","a");
...
if (track->GetTouchableHandle()->GetVolume() == fDetector->GetWorld()) {
 ...
 //if (IDp == 0) {G4cout << " pirmines pozicija [um] " << track->GetVertexPosition()/um << G4endl;} // vertex pozicija nurodo startavimo taska
  //G4Channeling* channeling = new G4Channeling();
  //G4ChannelingTrackData* chDat = new G4ChannelingTrackData();
  //chDat->Print();
  //auto tr = track;
  //G4double krit = channeling->GetCriticalAngle(tr);
  //G4cout << " Kritinis kampas: " << ar  << G4endl;


  G4double Trleng = track->GetTrackLength()+fPrimary->GetParticleGun()->GetParticlePosition().z()+0.5*fDetector->GetLength();

  if (track->GetParentID() == 0) {
    fEventAction->AddTrueTrakLen(Trleng);
    G4ThreeVector vertex = fPrimary->GetParticleGun()->GetParticlePosition(); // pakeista is GetParticlePosition();
    //G4ThreeVector bamba = vertex - fDetector->GetDimensions();
    G4ThreeVector position = track->GetPosition() + vertex;    //pakeista is   track->GetPosition() - vertex;
    //G4ThreeVector position = track->GetPosition() - bamba;
    fEventAction->AddProjTrakLen(position.z());
	} // is .mag() i .z()


 if (track->GetTrackID() == 1) {
   G4double z = track->GetPosition().z() + 0.5*fDetector->GetLength();
