...
// COPY FROM OLD CHANNELING
            ...
            ionIoni->SetStepFunction(0.1, 10*um); // org 10 um
            ...
            ecs_wrapper->RegisterProcess(ecs,0,2);	// perkeliau cia
        ...
        G4ProcessManager* pmanager = particle->GetProcessManager();
            ...
            //G4CoulombScattering* ecss = new G4CoulombScattering();
            //ecss->SetBuildTableFlag(false);
            //XWrapperDiscreteProcess *ecss_wrapper =
            //    new XWrapperDiscreteProcess();


            //ph->RegisterProcess(ecs_wrapper, particle);
/*
	    XWrapperDiscreteProcess *pnuc_wrapper =
                new XWrapperDiscreteProcess();
	    pnuc_wrapper->RegisterProcess(pnuc,1,1);
	    ph->RegisterProcess(pnuc_wrapper, particle); */


// multiple scattering nuimtas
/*
            // multiple scattering
...
/*
      G4ionIonisation* ionIoni = new G4ionIonisation();
      ionIoni->SetStepFunction(0.1, 20.*um); // originaliai 0.1, 10
...
// is older channeling
		...
		// prideta single scattering, jei amorfinis, vietoje multiple scattering

            //ph->RegisterProcess(ecs_wrapper, particle);

	    ph->RegisterProcess(pnuc,particle);

      //jofana broli kas cia per shudas
      G4ProcessManager* pmanager = particle->GetProcessManager();
...
/*
  	    G4NuclearStopping* pnuc_ion = new G4NuclearStopping();
	    XWrapperDiscreteProcess *pnuc_wrapper =
                new XWrapperDiscreteProcess();
	    pnuc_wrapper->RegisterProcess(pnuc_ion,0,1);
	    ph->RegisterProcess(pnuc_wrapper, particle); 
*/
/*
            // multiple scattering
