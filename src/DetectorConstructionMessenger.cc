...
:fTarget(mpga){
    ...
    // prideta kontrole del kristalo amorfizavimo    

    fXtalAmoCmd = new G4UIcmdWithABool("/xtal/SetAmorphous", this);
    fXtalAmoCmd->SetGuidance("Set crystal to amorphous");
    fXtalAmoCmd->SetParameterName("fCrystalAmorphous", true);
    fXtalAmoCmd->SetDefaultValue(false);

    fXtalLogCmd = new G4UIcmdWithAnInteger("/xtal/SetLogicalAmorphous", this);
    fXtalLogCmd->SetGuidance(" Choice: 0 = kristalinis loginis ; 1 = amorfinis loginis");
    fXtalLogCmd->SetParameterName("choice",true);
    fXtalLogCmd->SetRange("choice>=0");
    fXtalLogCmd->SetDefaultValue(0);
...
void DetectorConstructionMessenger::SetNewValue(
	...
	// mano testas blec
    if(command==fXtalAmoCmd) {
	fTarget->SetAmorphous(true);
	}

    if(command==fXtalLogCmd){
	fTarget->SetAmorph(fXtalLogCmd->GetNewIntValue(newValue));
	}
