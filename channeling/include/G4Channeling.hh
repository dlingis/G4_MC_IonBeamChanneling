...
private:
    ...
    //----------------------------------------
public:
    G4double GetCriticalAngle(const G4Track& aTrack){//G4cout << " total energy " << GetPre(aTrack)->GetTotalEnergy() << G4endl;
        return std::sqrt(2.0*GetMatData(aTrack)->GetPot()->GetMaxMin()
                         /GetPre(aTrack)->GetTotalEnergy());} //orgas total energy
...
private:
    ...
    //----------------------------------------
private:
    G4bool UpdateIntegrationStep(G4ChannelingMaterialData* materialData,
                                 const G4Track&,
                                 G4ThreeVector&,
                                 G4double&);
...
private:
    ...
    G4double fMaximumMomentumRatio;
    ...
    G4ChannelingMessenger* fMessenger;
};
